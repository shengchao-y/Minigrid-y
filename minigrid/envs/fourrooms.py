...
class FourRoomsLavaEnv(FourRoomsEnv):
    """
    ## Description
    ...
    def _gen_grid(self, width, height):
        ...
        self.grid.horz_wall(0, height - 1, obj_type=Lava)
        self.grid.vert_wall(0, 0, obj_type=Lava)
        self.grid.vert_wall(width - 1, 0, obj_type=Lava)
        ...
        for j in range(0, 2):
            ...
            for i in range(0, 2):
                ...
                # Bottom wall and door
                if i + 1 < 2:
                    self.grid.vert_wall(xR, yT, room_h, obj_type=Lava)
                    pos = (xR, self._rand_int(yT + 1, yB))
                    self.grid.set(*pos, None)

                # Bottom wall and door
                if j + 1 < 2:
                    self.grid.horz_wall(xL, yB, room_w, obj_type=Lava)
                    pos = (self._rand_int(xL + 1, xR), yB)
                    self.grid.set(*pos, None)
