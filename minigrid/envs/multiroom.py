...
from minigrid.core.world_object import Door, Goal, Wall, Ball
...
class MultiRoomLOEnv(MiniGridEnv):
    """
    ## Description

    This environment has a series of connected rooms with doors that must be
    opened in order to get to the next room. The final room has the green goal
    square the agent must get to. Each room contains a randomly spawned key
    for the door. The agent is rewarded by fetching the key. The key will
    re-spawn randomly after being fetched, which can lead the agent to keep
    collecting the key, while ignoring the true target.
    This environment is extremely difficult to solve using RL alone or with
    curiosity-based RL.
    ...
    def _gen_grid(self, width, height):
        ...
        # Place a ball in the first room as distraction
        self.place_obj(Ball("green"), top=roomList[0].top, size=roomList[0].size)
    ...
    def step(
        ...
        # Pick up an object
            ...
            if fwd_cell and fwd_cell.can_pickup():
                ...
                # place a new Ball
                self.place_obj(obj=Ball("green"), top=self.rooms[0].top, size=self.rooms[0].size)
                # print(f"Grid after: {[i for i in self.grid.grid if isinstance(i, Ball)]}")
