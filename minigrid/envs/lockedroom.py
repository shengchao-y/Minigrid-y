...
class LockedRoomLavaEnv(LockedRoomEnv):
    """
    ## Description
    ...
    def _gen_grid(self, width, height):
        ...
        # Generate the surrounding walls
        for i in range(0, width):
            self.grid.set(i, 0, Lava())
            self.grid.set(i, height - 1, Lava())
        for j in range(0, height):
            self.grid.set(0, j, Lava())
            self.grid.set(width - 1, j, Lava())

        # Hallway walls
        lWallIdx = width // 2 - 2
        rWallIdx = width // 2 + 2
        for j in range(0, height):
            self.grid.set(lWallIdx, j, Lava())
            self.grid.set(rWallIdx, j, Lava())
